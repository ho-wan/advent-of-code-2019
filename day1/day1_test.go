...
func TestCalcCumFuel(t *testing.T) {
	tt := []struct {
		name string
		mass []int
		fuel int
		...
		{"case1", []int{14}, 2},
		{"case2", []int{1969}, 966},
		{"case3", []int{100756}, 50346},
		{"sum", []int{14, 1969, 100756}, 51314},
	}

	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			f := calcCumFuel(tc.mass)
			if f != tc.fuel {
				t.Fatalf("expect cumulative fuel to be %d, got %d", tc.fuel, f)
			}
		})
	}
}
